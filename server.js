  ...
  if (req.method === 'POST' && req.url === '/webhook/lead-convert') {
    ...
    req.on('end', async () => {
      try {
        ...
        // Получаем сделку — ищем LEAD_ID из которого она создана
        const dealData = await b24Request('/crm.deal.get.json', { id: dealId });
        console.log('Deal LEAD_ID:', dealData.result?.LEAD_ID);

        const leadId = dealData.result?.LEAD_ID;
        if (!leadId) {
          console.log('No LEAD_ID in deal, skipping');
          res.writeHead(200);
          res.end('ok');
          return;
        }
        ...
        console.log('Lead file field:', JSON.stringify(leadData.result?.UF_CRM_1781144284968));
