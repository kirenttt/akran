...
const server = http.createServer((req, res) => {
  ...
  // --- Webhook от Битрикс24: создана сделка (ONCRMDEALADD) ---
  if (req.method === 'POST' && req.url === '/webhook/lead-convert') {
    ...
    req.on('end', async () => {
      try {
        ...
        console.log('Webhook dealId:', dealId);
        res.writeHead(200);
        res.end('ok');
      } catch (err) {
  ...
  req.on('end', async () => {
    try {
      ...
      if (fileKey) {
        ...
        // Получаем id файла из лида
        const leadData = await b24Request('/crm.lead.get.json', { id: leadId });
        const fileField = leadData.result?.UF_CRM_1781144284968;
        fileId = Array.isArray(fileField) ? fileField[0]?.id : fileField?.id;
        console.log('File id:', fileId);
      }

      // 3. Конвертируем лид в сделку
      console.log('Converting lead to deal...');
      const convertResult = await b24Request('/crm.lead.convert.json', {
        id: leadId,
        fields: {},
        deals: [{ fields: {} }]
      });
      ...
      // 4. Копируем файл в сделку если есть
      const dealId = convertResult.result?.deals?.[0]?.id;
      console.log('Deal id:', dealId);

      if (dealId && fileId) {
        const dealFileResult = await b24Request('/crm.deal.update.json', {
          id: dealId,
          fields: {
            UF_CRM_1769057513043: { id: fileId }
          }
        });
        console.log('Deal file result:', JSON.stringify(dealFileResult));
      }

      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, leadId, dealId: dealId || null }));
